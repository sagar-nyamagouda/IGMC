...
if __name__ == '__main__':
    ...
    if args.transfer:
        if args.data_name in ['flixster', 'ml_10m', 'openml']:  # original 0.5, 1, ..., 5
            post_rating_map = {
    ...
    if args.data_name in ['flixster', 'douban', 'yahoo_music']:
        ...
        print(f"the data name is {args.data_name}")
        print(f" testing is {args.testing}")
        print(f"datasplit_path is {datasplit_path}")
        print(f"rating map is {rating_map}")
        print(f"post rating map is {post_rating_map}")
        print(f"u_features is {u_features}")
        print(f"v_features is {v_features}")
        print(f"adj_train is {adj_train}")
        print(f"train_u_indices is {train_u_indices}")
        print(f"train_v_indices is {train_v_indices}")
        print(f"train_labels are {np.unique(train_labels)}")
        print(f"test_labels are {np.unique(test_labels)}")
        print(f"Validation labels are {np.unique(val_labels)}")
    ...
    if args.use_features:
        ...
        elif u_features is not None:
            u_features = u_features.toarray()
            n_features = u_features.shape[1]
            print('Number of task features {} and total features {}'.format(
                u_features.shape[1], n_features))
            v_features = None
        else:
            v_features = v_features.toarray()
            n_features = v_features.shape[1]
            print('Number of flow features {} and total features {}'.format(
                v_features.shape[1], n_features))
            u_features = None
    else:
    ...
    print(f"adjacency graph is {adj_train}")
    ...
    if args.visualize:
        ...
        if args.ensemble:
            ...
            elif args.data_name == 'openml' or args.data_name == 'openmlV2':
                start_epoch, end_epoch, interval = args.epochs - 10, args.epochs, 10
            else:
